...
The portal serves its captcha as an SVG file with characters embedded directly
in <text> elements. We simply parse the XML — no OCR, no external API needed.
"""
...
def extract_from_svg(svg_content: str | bytes) -> str:
    ...
    Each character is a plain <text> element, e.g.:
        <text x="22" y="36" ...>H</text>

    Characters are sorted by their x attribute to get left-to-right order.
    """
    if isinstance(svg_content, bytes):
        svg_content = svg_content.decode("utf-8", errors="ignore")

    matches = re.findall(
        r'<text\s[^>]*\bx=["\'](\d+(?:\.\d+)?)["\'][^>]*>([^<]+)</text>',
        svg_content,
    )

    if not matches:
        raise CaptchaSolveError(
            "No <text> elements found in captcha SVG — portal may have changed format",
