      ...
      >
        ...
        <nav className="flex-1 px-3 pb-3 pt-3">
          <ul className="space-y-1">
            {navItems.map((item) => (
              <li key={item.to} data-walkthrough={item.to.endsWith('/history') ? 'sidebar-history' : item.to.endsWith('/study') ? 'sidebar-study' : item.to.endsWith('/notices') ? 'sidebar-notices' : item.to.endsWith('/marks') ? 'sidebar-marks' : undefined}>
                {item.to.endsWith('/study') ? (
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        {item.useInlineSvg === 'arcade' ? (
                          <Gamepad2
                            className={`shrink-0 ${
                              isActive ? 'drop-shadow-[0_0_8px_rgba(255,145,108,0.45)]' : 'opacity-70'
                            }`}
                            style={{ width: '18px', height: '18px' }}
                            aria-hidden="true"
                          />
        ...
        <div className="border-t border-white/10 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <a href="https://www.instagram.com/attend.75/" target="_blank" rel="noopener noreferrer" className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#3D3660] border border-white/10 text-[#D8D4E7] transition hover:border-[#E1306C]/40 hover:text-[#E1306C]">
              <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="5" ry="5" /><circle cx="12" cy="12" r="4" /><circle cx="17.5" cy="6.5" r="1.2" /></svg>
            </a>
          </div>
        </div>
